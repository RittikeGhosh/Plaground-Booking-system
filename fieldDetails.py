...
def call_description(email, uid):
    ...
    frame.pack()
    head = Label(frame, text=x[0][1], font=("Times new roman", 20))
    head.grid(row=1)
    # fname = i[7]
    ...
    canvas = Canvas(frame_can, width=600, height=300)
    imgLabel = Label(canvas,image = img)
