...
fname = "cartoon_birds_blue_flying_animation_clipart.gif"
...
fname ="71JnoSPl2hL._SL1134_ (1).gif" 
bg_image=PhotoImage(file=fname)
w=bg_image.width()
h = bg_image.height()
window.geometry("%dx%d"%(w+100,h+100))
canvas = Canvas(window,width=w+50, height=h+50)
canvas.pack()
canvas.create_image(0, 0, image=bg_image,anchor="nw")
#l1=Label(canvas,text="user name :")
...
def signup():
    ...
    def verify():#called on clicking register button
        ...
        if f1==1 and f2==1 and f3==1 and f35==1 and f4==1 and f5==1:
            ...
            #n=sendMail(en5.get()) #admin function call
            ...
            frame1=Frame(email,height=400,width=800,bg="grey",padx=50,pady=50)
            frame1.pack()
            m=Message(frame1,text="You have recieved a confirmation mail. Please enter the OTP below to confirm the email id.",width=600)
            ...
            def success():#when signup is successful it is added to the list which could further be updated in the database
                if(entry.get()==n):
                    ...
                    details.append(l)
                    print (details)
                    messagebox.showinfo("Information","Congratualtions! You have successfully resgistered....!!!")
            ...
            sp1.grid(row=4)
            submit=Button(frame1,text="submit",bg="green",fg="white",command=success)
    ...
    register=Button(frame,text="RESGISTER",bg="red",fg="white",command=verify)
...
admin=Button(frame,bg="black",fg="white",text="ADMIN LOGIN",anchor="e",command=adminlogin())
...
sign=Button(frame,bg="black",fg="white",text="SIGN UP",anchor="w",command=signup)
