        ...
        if f1==1 and f2==1 and f3==1 and f35==1 and f4==1 and f5==1:
            ...
            def success():#when signup is successful it is added to the list which could further be updated in the database
                if(entry.get()==n):
                    ...
                    details.append(l)
                    print (details)
                    database.execute("insert into user_details values (?,?,?,?,?,?)",l)
                    database.commit()
                    database.close()
        ...
        for i in x:
            if i[4]==e1.get():
                if(e2.get()==""):
                    ...
                    messagebox.showwarning("Warning","You have entered the wrong password or this username does not exist!!!")
    database.commit()
    database.close()
        #for i in details:
            #if(e1.get()==i[5]):
...
def adminlogin():#called on clickin the admin login button. To be edited by the admin page creator
    pass;#ritik's module will be called
